...
function dims(upm: number, scale:number) {
  return {
    routeLineW:  0.35 * upm * 15000 / scale,
    routeXArm:   1.5  * upm * 15000 / scale,
    routeXW:     0.35 * upm * 15000 / scale,
    routeXSpace: 5.0  * upm * 15000 / scale,
    crossW:      0.6  * upm * 15000 / scale,
    crossHalf:   1.5  * upm * 15000 / scale,
    crossH:      1.5  * upm * 15000 / scale,
    hatchSpace:  1.2  * upm * 15000 / scale,
    hatchW:      0.2 * upm * 15000 / scale,
    boundaryW:   0.7  * upm * 15000 / scale,
  }
}
...
function ForbiddenRoute({ points, upm, scale, color }: {
  points: MapPoint[]; upm: number; scale: number; color: string
}) {
  if (points.length < 2) return null
  const d = dims(upm,scale)
...
function CrossingPoint({ center, upm, scale, rotation, color }: {
  center: MapPoint; upm: number; scale: number; rotation: number; color: string
}) {
  const { x, y } = center
  const d = dims(upm, scale)
...
function OutOfBoundsArea({ points, upm, scale, color, patternId }: {
  points: MapPoint[]; upm: number; scale: number; color: string; patternId: string
}) {
    ...
    <g style={{ pointerEvents: 'none' }}>
      {annotations.map((ann, idx) => {
        ...
        if (ann.type === 'forbidden_route') {
          return (
            <g key={ann.id}>
              <ForbiddenRoute points={ann.points} upm={upm} scale={scale} color={color} />
            </g>
          )
        }
      ...
      {pendingPoints.length > 0 && pendingType === 'forbidden_route' && (
        <g opacity={0.5}>
          <ForbiddenRoute points={pendingPoints} upm={upm} scale={scale} color={color} />
        </g>
      )}
