...
**Rendering is pure SVG.** `MapCanvas` owns the viewport (pan/zoom) and all pointer/touch/wheel event handling via native listeners in a single `useLayoutEffect` with `[]` deps. It reads store actions once (Zustand action refs are stable) and reads live state via `useStore.getState()` inside handlers to avoid stale closures. The SVG layers stack: MapLayer (base map) → LegsLayer (course lines) → AnnotationsLayer (forbidden routes, crossing points) → ControlsLayer (symbols + labels).
